...
OUT=Path('output/report/drawio-erd')
...
layout=[
 ['roles','user_profiles','users','endurance_tests'],
 ['notifications','chat_messages','progress_tracking','service_ratings'],
 ['memberships','invoices','pet_profiles','user_cosmetic_ownership'],
 ['support_messages','support_sessions','work_shifts','customer_addresses'],
...
STEP=20; W=520; GAP=340; LEFT=280; TOP=180
rect={}; y=TOP
for row in layout:
...
def route(start,end):
    ...
    while heap:
        ...
        for dx,dy,nd in [(1,0,0),(-1,0,0),(0,1,1),(0,-1,1)]:
            ...
            crossing=18 if q in through else 0
...
for n,(x,y,w,h) in rect.items():
    ...
    for i,(key,text) in enumerate(attrs[n]):
        c=cell(n+'_row_'+str(i),(key+'   ' if key else '')+text,'text;html=0;align=left;verticalAlign=middle;whiteSpace=wrap;overflow=hidden;spacingLeft=10;fontFamily=Arial;fontSize=13;strokeColor=none;fillColor=none;',n)
...
for index,pts in routes:
    ...
    style=f'edgeStyle=none;rounded=0;html=0;strokeColor=#333333;strokeWidth=1.2;jumpStyle=arc;jumpSize=8;startArrow={markers[left]};endArrow={markers[right]};startSize=16;endSize=16;startFill=0;endFill=0;dashed={int(dashed)};exitX={(sx-ax)/aw};exitY={(sy-ay)/ah};exitPerimeter=0;entryX={(tx-bx)/bw};entryY={(ty-by)/bh};entryPerimeter=0;'
